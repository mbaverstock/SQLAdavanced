...
-- return first date of cuurretn month
SELECT DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1) AS First

--rethen last day of current meiontj
-- SELECT DATEFROMPARTS(2026, 03, 01) AS TheDate;

-- FORMAT returns a string given a data and a format specifier 
SELECT FORMAT(DATEFROMPARTS(2022, 1, 18), 'dddd ddd dd-MMMM MMM MM -yyyy');
...
SELECT DATEADD(MONTH, -2, '2022-04-30');
...
SELECT DATEFROMPARTS(DATEPART(YEAR, GETDATE()), DATEPART(MONTH, GETDATE()), 1) AS TheDate;



select right(t.PostCode,3) from (
select 'SW12 0EN' as PostCode
UNION
select 'w1a 1aa' as t)
