...
SELECT
	...
	,ps.AdmittedDate
	,ps.Tariff
	,ROW_NUMBER() OVER (ORDER BY ps.tariff DESC,ps.patientid DESC) AS PatientIndex
...
SELECT
	...
	,AdmittedDate
	,Tariff
	,ROW_NUMBER() OVER (PARTITION BY MonthAdmitted ORDER BY AdmittedDate) AS RowIndex
	,SUM(Tariff) OVER (PARTITION BY MonthAdmitted ORDER BY AdmittedDate) AS RunningTariff
FROM
	#TempTable;
